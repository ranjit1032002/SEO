I now have all the data needed. Compiling the full page analysis.
...
First Byte: The page took 5.77s to fully deliver — measured from a local curl request, meaning real-user LCP on mobile India
will be substantially worse. Stock pages are time-sensitive and users expect near-instant price visibility; slow TTFB will hurt engagement.
...
The page has zero twitter:card, twitter:title, twitter:image meta tags. When SBIN share price is shared on X/Twitter (common for traders), the link will render as a bare URL — no preview, no engagement.
...
The second JSON-LD block uses "@type": "Webpage" — the correct "WebPage". Schema.org is case-sensitive. Google's Rich Results parser will silently fail to parse this type, forfeiting any entity association benefit.
...
This renders in Google breadcrumb rich results as "Sbin share price" — inconsistent capitalisation and ticker formatting. It should match the display heading.
...
The page sets og:type as "Website" (capital W). Use lowercase: "website". Some crawlers and social preview tools may reject capitalised values.

Fix: Change to content="website".

---
6. No dateModified on WebPage Schema (Critical)

Financial data pages must signal freshness. There is no dateModified property, and there is no visible "Last Updated" timestamp on the page itself. For a YMYL stock page where data changes intraday, Google's QRG specifically looks for update signals to assess content freshness and E-E-A-T.
...
This will truncate in desktop SERPs (~600px). It repeats twice ("SBI Share Price" and "State Bank of India Share Price"), wasting crawl space on a repeat keyword.

Suggested title (58 chars):
SBI Share Price Live | State Bank of India NSE/BSE

Or if brand name is in suffix:
SBI Share Price Today — Live NSE/BSE | 5paisa (47 chars, punchy)

---
8. Missing Corporation Schema for State Bank of India

The page describes SBIN the company, but there is no Corporation schema for SBI itself — only a FinancialProduct schema that names the page, not the entity. Competitors like Moneycontrol use structured Corporation schema which powers knowledge panel associations for the entity.
...
This strengthens entity disambiguation and AI Overview citation eligibility.

---
9. FAQPage Schema — No Longer Generates Rich Results

The page uses FAQPage schema with 4 Q&As. As of recent changes, FAQ rich results are limited and may not appear as expandable accordions in SERPs. However, the structured Q&A retains value as an AI Overview and LLM citation signal — keep the schema, do not remove it, but revisit the choice of questions.

The 4 questions (Share Price, Market Cap, P/E, intrinsic value) — their schema answers will become stale between crawls. Consider whether static FAQ answers serve better (e.g., "How to invest in SBI shares?") for durable AI citations.

---
10. OG:Image Uses a 96×96px Company Icon Logo

The og:image points to https://images.5paisa.com/MarketIcons/SBIN.png — the same 96×96 stock ticker icon used in the UI. Social platforms require a minimum of 600×315px for proper Open Graph image previews. A tiny logo will render as a micro-thumbnail or be discarded entirely by platforms like WhatsApp, LinkedIn, and Facebook.
...
Without explicit dimensions, the browser cannot reserve layout space, causing Cumulative Layout Shift when the SVG loads. For a page with live price data already prone to layout shifts, this increases CLS risk.

Fix: Add width and height (even if width="auto" with a fixed height) or CSS aspect-ratio to reserve space.

---
12. cache-control: public With No Explicit max-age

The SBIN page response headers show cache-control: public without an explicit max-age. The Expires header gives ~10 minutes. For a stock detail page served at scale (thousands of requests), this short cache window means frequent cache misses and Drupal re-renders — contributing to the slow TTFB. The static HTML shell (nav, schema, about section) can safely be cached for 30–60 minutes; only the live price widget needs frequent updates.

Fix: Set cache-control: public, max-age=1800 for HTML shell, and load live prices via an XHR that bypasses the HTML cache.
...
From a user and Google QRG perspective, YMYL financial pages should clearly display when data was last refreshed. The FAQ section contains "As on 20 June, 2026 | 20:16" but there is no prominent page-level timestamp (e.g., in the header area near the price) visible to crawlers in a <time datetime="..."> element.

---
14. meta name="robots" Tag Absent

While the absence of a robots tag defaults to index, it's best practice for programmatic pages to explicitly declare this, especially on a Drupal CMS where module misconfiguration could accidentally inject noindex.

Fix: Add <meta name="robots" content="index, follow, max-snippet:-1, max-video-preview:-1, max-image-preview:large" /> (same as homepage).
...
├─────┼──────────────────────────────────────────────────────────┼──────────┼────────────────┤
│ 13  │ No visible <time> timestamp for last data               │ Content        │
├─────┼──────────────────────────────────────────────────────────┼──────────┼────────────────┤
...
- Hreflang is properly implemented across 5 languages (en-IN, hi-IN, mr-IN, bn-IN, x-default)
...
- External links to authoritative sources (Business Line) — strong E-E-A-T link signal
...
- Similar Stocks section with descriptive anchor text ("Compare: SBI Share Price") creates strong internal link equity

---
Schema Additions — Ready to Use

Fix 1: Correct WebPage Schema (fixes capitalisation and adds dateModified)
...
Fix 3: Add Corporation Schema for SBI Entity
...
- Schema: Google Rich Results Test → expect BreadcrumbList to pass cleanly with zero errors
...
Track GSC CTR for /stocks/*-share-price template pages. Schema fixes + Twitter Card will improve click-through; performance fixes will improve mobile ranking position.
